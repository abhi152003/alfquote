...
| Skill reference contract (WO-17) | **Pinned public reference + documented checkout** — clone `https://github.com/abhi152003/alfquote` at tag `phase2-baseline`, then `npm ci && npm run build`, and run the CLI from the workspace (`node packages/cli/dist/main.js …` or `npm exec --workspace=@alfquote/cli -- alfquote …`; bare `npx alfquote` is NOT reliable after a fresh `npm ci` — npm does not create the `node_modules/.bin` link). npm publication stays optional/out of scope. **The npm Git-dependency form `github:abhi152003/alfquote#phase2-baseline` is NOT a valid install contract**: npm resolves a Git dependency from the repository *root* `package.json`, which here is the private workspace root with no exports and no bin, and npm cannot select `packages/alfquote` / `packages/cli` from a Git URL. The skill must treat ALFQuote as a pinned public reference and documented CLI checkout, never as an npm dependency. | Orchestrator correction (WO-16 review); install steps verified credential-free 2026-09-12 (see review-log Round 2) |
